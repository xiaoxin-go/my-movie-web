  ...
  <div class="main">
    <div class="main-title1">
      <Breadcrumb>
        ...
        <BreadcrumbItem>修改角色</BreadcrumbItem>
      </Breadcrumb>
    </div>
...
<script>
  import {getRole, putRole} from '../../api/index.js';
  export default {
    created(){
      this.getData()
    },
    ...
    methods: {
      ...
      async getData(){
        let resp = await getRole({id: this.id});
        this.name = resp.data[0].name
      },

      // 添加角色
      async update(){
        if (this.name === null){this.$Message.warning('请输入角色名称');return}
        let json_data = {
          id: this.id,
          name: this.name,
        };
          let resp = await putRole(json_data);
          if (resp.state === 1){
            this.$Message.success('角色修改成功');
            this.$router.push('/admin/role')
          } else if(resp.state === 2){
            this.$Message.warning('修改失败，角色已存在')
          }else{
